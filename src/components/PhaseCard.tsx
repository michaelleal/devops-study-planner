...
export const PhaseCard = ({ phase }: PhaseCardProps) => {
  ...
  return (
    <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow bg-white">
      {/* Phase Header */}
      <div className={`${phase.color} text-white p-4 relative`}>
      ...
      {phase.isLocked ? (
        <div className="px-4 py-6 bg-gray-50 text-center">
          <p className="text-sm text-gray-600 mb-2">🔒 Esta fase está bloqueada</p>
          <p className="text-xs text-gray-500">
            Complete {phase.requiredProgress}% da fase anterior para desbloquear
          </p>
        </div>
      ) : (
        <>
